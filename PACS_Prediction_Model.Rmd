...
### Versions were checked with *packageVersion().*

# TO_DO 
# checkpoint {checkpoint} cannot currently function correctly with the remotely installed libraries.

```{r}
# Packages requiring remote installation

# install.packages("remotes")
library("remotes", quietly = TRUE)                    #version 2.4.2
# packageVersion("remotes")

# install.packages("biostatUZH", repos = "http://R-Forge.R-project.org")
library("biostatUZH", quietly = TRUE)                 #version 1.8.0
# packageVersion("biostatUZH")


library("CalibrationCurves", quietly = TRUE)          #version 0.1.2
# packageVersion("CalibrationCurves")
...
# create_checkpoint("2022-04-19",
#                   r_version = current_r_version,
#                   project_dir = "/Users/macminim1/Documents/GitHub/PACS_Prediction_Model")
# use_checkpoint("2022-04-19", r_version = current_r_version)


# Libraries installed with install.packages() {utils}:
library("dcurves", quietly = TRUE)                    #version 0.2.0
# packageVersion("dcurves")
...
library("ggstatsplot", quietly = TRUE)                #version 0.9.1
# packageVersion("ggstatsplot")

library("grid", quietly = TRUE)                       #version 4.1.3
# packageVersion("grid")

library("interactions", quietly = TRUE)               #version 1.1.5
# packageVersion("interactions")

library("mgcv", quietly = TRUE)                       #version 1.8.40
# packageVersion("mgcv")

library("pROC", quietly = TRUE)                       #version 1.18.0
# packageVersion("pROC")

library("readxl", quietly = TRUE)                     #version 1.4.0
# packageVersion("readxl")

library("rstudioapi", quietly = TRUE)                 #version 0.13
# packageVersion("rstudioapi")

library("scales", quietly = TRUE)                     #version 1.2.0
# packageVersion("scales")

library("shrink", quietly = TRUE)                     #version 1.2.2
# packageVersion("shrink")

library("sjPlot", quietly = TRUE)                     #version 2.8.10
...
xdata <-
  read_excel(
    "~/Documents/GitHub/PACS_Prediction_Model/41467_2021_27797_MOESM5_ESM.xlsx",
    na = "NA"
  )
...
healthy <-
  xdata %>% filter(SevMax2 == "Healthy") %>% mutate(Sampling_month = "SIX",
                                                    Complete_OST = "NO")
...
### 2b.1 Followed-up patients

```{r}
# Followed-up patients
xdata_selectR <- xdata[xdata$Resampling %in% c("YES"), ]
```

### 2b.2 Followed-up patients + healthy controls

```{r}
# Followed-up patients + healthy controls
xdata_selectR2 <- xdata[xdata$Resampling_andH %in% c("YES"), ]
```

### 2b.3 Primary infection

```{r}
# Primary infection
xdata_selectONE <- xdata[xdata$Sampling_month %in% c("ONE"), ]
...
### 2.b.5 Primary infection & 6-month follow-up
...
### 2.b.6 Without Healthy controls
...
### 2.b.8 Patients with history of asthma
...
### 2.b.11 Complete observations: Patients with blood sampling at all three patient visits

```{r}
# Complete observations: Patients with blood sampling at all three patient visits
xdata_selectRcOST <-
